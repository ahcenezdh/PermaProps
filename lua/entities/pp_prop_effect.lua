...
local ents_FindByClassAndParent = ents.FindByClassAndParent
local render_SetMaterial = CLIENT and render.SetMaterial
local table_Copy = table.Copy
local duplicator_CopyEntTable = SERVER and duplicator.CopyEntTable
local duplicator_DoGeneric = SERVER and duplicator.DoGeneric
local duplicator_ApplyEntityModifiers = SERVER and duplicator.ApplyEntityModifiers
local duplicator_ApplyBoneModifiers = SERVER and duplicator.ApplyBoneModifiers
...
function ENT:Initialize()
    ...
    if SERVER then
        self.AttachedEntity = ents_Create("prop_dynamic")
        ...
        local tab = ents_FindByClassAndParent("prop_dynamic", self)
...
-----------------------------------------------------------]]
function ENT:Draw()
    render_SetMaterial(self.GripMaterial)
end
...
function ENT:OnEntityCopyTableFinish(tab)
    ...
    tab.AttachedEntityInfo = table_Copy(duplicator_CopyEntTable(self.AttachedEntity))
...
function ENT:PostEntityPaste(ply)
    ...
    if IsValid(self.AttachedEntity) and self.AttachedEntityInfo then
        ...
        duplicator_DoGeneric(self.AttachedEntity, self.AttachedEntityInfo)
        if self.AttachedEntityInfo.EntityMods then
            self.AttachedEntity.EntityMods = table_Copy(self.AttachedEntityInfo.EntityMods)
            duplicator_ApplyEntityModifiers(ply, self.AttachedEntity)
        end

        if self.AttachedEntityInfo.BoneMods then
            self.AttachedEntity.BoneMods = table_Copy(self.AttachedEntityInfo.BoneMods)
            duplicator_ApplyBoneModifiers(ply, self.AttachedEntity)
        end
