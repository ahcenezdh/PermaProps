...
PermaProps.SpecialENTSSpawn["prop_ragdoll"] = function(ent, data)
    ...
    if data["Bones"] then
        for objectid, objectdata in pairs(data["Bones"]) do
            ...
            if isvector(objectdata.Pos) and isangle(objectdata.Angle) then
                ...
                Phys:SetAngles(ang)
                Phys:Wake()
                if objectdata.Frozen then Phys:EnableMotion(false) end
            end
        end
    end
    ...
    if data["Flex"] and ent:IsValid() then
        ...
        if Scale then ent:SetFlexScale(Scale) end
    end
    return true
end
...
PermaProps.SpecialENTSSpawn["NPC"] = function(ent, data)
    if data and istable(data) then
        if data["Equipment"] then
            local valid = false
            for _, v in pairs(list_Get("NPCUsableWeapons")) do
                if v.class == data["Equipment"] then
                    valid = true
                    break
                end
            end
...
if list_Get("NPC") and istable(list_Get("NPC")) then
    for k, v in pairs(list_Get("NPC")) do
...
PermaProps.SpecialENTSSpawn["item_ammo_crate"] = function(ent, data)
    if data and istable(data) and data["type"] then
        ent.type = data["type"]
        ent:SetKeyValue("AmmoType", math_Clamp(data["type"], 0, 9))
    end
...
PermaProps.SpecialENTSSave["prop_ragdoll"] = function(ent)
    ...
    if ent:HasBoneManipulations() then
        ...
        for i = 0, ent:GetBoneCount() do
            ...
            if s ~= Vector(1, 1, 1) then -- scale
                t['s'] = s
            end

            if a ~= Angle(0, 0, 0) then -- angle
                t['a'] = a
            end

            if p ~= Vector(0, 0, 0) then -- position
                t['p'] = p
            end

            if table_Count(t) > 0 then content.Other["BoneManip"][i] = t end
        end
    end
